...
# --- SISTEMA DE REFORMULACIÓN DE CONSULTAS (QUERY REWRITER) ---
...
# --- PROMPT MEJORADO CON SOPORTE PARA ANALÍTICA VOC (VOICE OF CUSTOMER) Y FALLBACKS ---
prompt_template = ChatPromptTemplate.from_messages([
    ("system", """Eres un asistente analítico experto en análisis de Voz del Cliente (VoC) y reseñas de productos.
...
1. **Responde de forma natural, clara y directa:** Adapta tu tono y redacción completamente a lo que el usuario esté preguntando (sea sobre los mejores productos, quejas, sentimientos, problemas específicos o estadísticas).
...
3. **Fuente de verdad y Metadatos:** Tu fuente principal y absoluta es el <contexto_de_base_de_datos>. Si el contexto incluye etiquetas de Sentimiento (positive, neutral, negative), Categorías o Tags de aspectos, aprovéchalos para ofrecer respuestas analíticas más precisas.
...
7. **Criterio de redacción inteligente según la intención:**
   ...
   - Si el usuario pregunta por **quejas, problemas específicos, sentimientos negativos o fallos**, extrae o resume los inconvenientes mencionados en las reseñas de forma directa.
...
async def stream_insight(context: str, question: str, chat_history: list = [], sources: list = None):
    ...
    if sources:
        # 🛡️ PROTECCIÓN CRÍTICA: default=str evita fallos de serialización con datetimes/UUIDs de Supabase
